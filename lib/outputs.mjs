...
export function engineeredPrompt(s) {
  ...
  // Auto-fill one few-shot example from the idea's own persona / entity / task,
  // so it's a useful starting pair rather than a bare placeholder.
  const p0 = (s.personas[0] || 'user').toLowerCase().replace(/s$/, '');
  const e0raw = s.entities[0] && !/none/i.test(s.entities[0]) ? s.entities[0] : 'input';
  const e0 = e0raw.toLowerCase();
  const r0 = (s.functional_requirements[0] || 'the primary task').replace(/^./, (c) => c.toLowerCase());
...
## Few-shot example (auto-filled from your idea — swap in a real pair when you have one)
- Example input: A ${p0} provides a ${e0} — e.g. {{a concrete ${e0}}}.
- Example output: The result of "${r0}", rendered in the output format above.
