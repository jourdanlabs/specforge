...
function draftMessages({ idea, industry, type, outputs }) {
  ...
  return [
    {
      role: 'system',
      content:
        'You are a principal engineer who turns vague product ideas into precise, build-ready specifications. ' +
...
export async function aiSpec({ idea, industry, type, outputs }) {
  const { draft: draftModel, validate: validateModel } = cfg();

  const draft = await chat(draftMessages({ idea, industry, type, outputs }), draftModel);
