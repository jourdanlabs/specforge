  ...
  return [
    ...
    },
    {
      ...
      content:
        ...
        (wanted.includes('Engineered Prompt')
          ? `For "Engineered Prompt", apply prompt-engineering technique: a reusable prompt with Role, ` +
            `Objective, Context, explicit numbered Instructions, Constraints, an Output-format contract, ` +
            `a self-check list, a {{INPUT}} placeholder for per-run input, and ONE fully worked few-shot ` +
            `example (a realistic input and the ideal output) drawn from this idea's own domain.`
          : ''),
    },
  ];
}
